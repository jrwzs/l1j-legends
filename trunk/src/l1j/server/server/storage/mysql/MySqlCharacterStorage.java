...
public class MySqlCharacterStorage implements CharacterStorage {
	...
	public L1PcInstance loadCharacter(String charName) {
		...
		try {
			...
			pstm = con
					.prepareStatement("SELECT *, (select access_level from accounts where login = characters.account_name) as isGM FROM characters WHERE char_name=?");
			...
			pc.setAccessLevel(rs.getShort("isGM"));
            ...
            pc.setPvpRank(rs.getInt("PvpRank"));
