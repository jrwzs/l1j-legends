...
public class L1Magic {
    ...
    // adding DK debuff so it won't be able to cast it in safe zone - [Hank]
        ...
        if ((_pc != null) && (_targetPc != null)) {
            if ((_pc.getZoneType() == 1) || (_targetPc.getZoneType() == 1)) {
                if ((skillId == WEAPON_BREAK) || (skillId == SLOW) || (skillId == CURSE_PARALYZE) || (skillId == MANA_DRAIN) || (skillId == DARKNESS)
                        ...
                        || (skillId == ICE_LANCE_COCKATRICE) || (skillId == ICE_LANCE_BASILISK) || (skillId == SILENCE) || (skillId == GUARD_BRAKE) ||
                        (skillId == RESIST_FEAR) || (skillId == SHOCK_SKIN) || (skillId == THUNDER_GRAB) || (skillId == HORROR_OF_DEATH)) {
                    return false;
                }
            }
        }
