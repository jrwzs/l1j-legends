			...
			if (obj instanceof L1NpcInstance) {
				...
				if (s.matches("[0-9]+")) {
					...
					if ((awakeSkillId == AWAKEN_ANTHARAS)
						...
						target.sendPackets(new S_ServerMessage(1384)); // ç�¾åœ¨ã�®çŠ¶æ…‹ã�§ã�¯å¤‰èº«ã�§ã��ã�¾ã�›ã‚“ã€‚
						return;
					}
...
//						l1skilluse.handleCommands(pc,ADVANCE_SPIRIT , pc.getId(), pc.getX(),pc.getY(), null, 0, L1SkillUse.TYPE_SPELLSC);
						break;
					}
				}
			}
			...
			int[] allBuffSkill =
				{LIGHT, DECREASE_WEIGHT, PHYSICAL_ENCHANT_DEX, MEDITATION, PHYSICAL_ENCHANT_STR, BLESS_WEAPON, ADVANCE_SPIRIT, IRON_SKIN};
//			for (L1ItemInstance item : pc.getInventory().getItems()) {
//				if (pc.getWeapon().equals(item)) {
//					L1SkillUse l1skilluse = new L1SkillUse();
//					l1skilluse.handleCommands(pc,ADVANCE_SPIRIT , pc.getId(), pc.getX(),pc.getY(), null, 0, L1SkillUse.TYPE_SPELLSC);
...
//							L1Quest.QUEST_END);
		...
		// ç„¶æŸ³å¯µç‰©å•†
		else if (((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 70077 // ç¾…å¾·å°¼
				|| ((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 81290) { // ç�­é…·
			int consumeItem = 0;
			int consumeItemCount = 0;
			int petNpcId = 0;
			int petItemId = 0;// 40314 ä½Žç­‰å¯µç‰©é …åœˆ
			int upLv = 0; // ç­‰ç´š
			int lvExp = 0; // LV.upLv ç¶“é©—å€¼
			String msg = "";
			if (s.equalsIgnoreCase("buy 1")) {
				petNpcId = 45042;// æ�œè³“ç‹—
			...
			if (petNpcId > 0) {
				if (!pc.getInventory().checkItem(consumeItem, consumeItemCount)) {
					...
					pc.sendPackets(new S_ServerMessage(337, "èº«ä¸Šç©ºé–“"));
				...
				pc.sendPackets(new S_SystemMessage("å°�è©±æª”ç‰ˆæœ¬ä¸�ç¬¦ï¼Œè«‹ä¸‹è¼‰æ›´æ–°"));
			}
			htmlid = "";
		}
