...
//		_ac = IntRange.ensure(i, -212, 211);
    ...
    private int getScaledLaw(int i)
    {
        int newLaw;
        if(this.getLevel() > 1 && this.getLevel() <= 30)
        {
            newLaw = i*10;
        }
        else if (this.getLevel() > 30 && this.getLevel() <= 52)
        {
            newLaw = i*5;
        }
        else if (this.getLevel() > 52 && this.getLevel() <= 60)
        {
            newLaw = i;
        }
        else if (this.getLevel() > 60 && this.getLevel() <= 69)
        {
            newLaw = (int) Math.floor(i*0.75);
        }
        else if (this.getLevel() >= 70 && this.getLevel() <= 74)
        {
            newLaw = (int) Math.floor(i*0.5);
        }
        else if (this.getLevel() >= 75)
        {
            newLaw = (int) Math.floor(i*0.25);
        }
        else
        {
            newLaw = i;
